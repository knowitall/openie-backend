...
object LzoTextInput {
  lazy val logger = LogFactory.getLog("scoobi.LzoTextInput")
  ...
  def fromLzoTextFile(paths: String*): DList[String] = fromLzoTextFile(List(paths:_*))
  ...
  def fromLzoTextFile(paths: List[String]): DList[String] = {
    ...
    DList.fromSource(new LzoTextSource(paths, converter))
  }
  ...
  /** Create a distributed list from one or more files or directories (in the case of
    ...
    * text. */
  def fromLzoTextFileWithPath(paths: String*): DList[(String, String)] = fromLzoTextFileWithPath(List(paths:_*))
  ...
  /** Create a distributed list from a list of one or more files or directories (in the case of
    ...
    * where the first part is the path of the originating file and the second part is a line of
    * text. */
  def fromLzoTextFileWithPath(paths: List[String]): DList[(String, String)] = {
  ...
  def fromDelimitedLzoTextFile[A : Manifest : WireFormat]
      (path: String, sep: String = "\t")
      (extractFn: PartialFunction[List[String], A])
    : DList[A] = {

    val lines = fromLzoTextFile(path)
    lines.flatMap { line =>
  ...
  class LzoTextSource[A : Manifest : WireFormat](paths: List[String], converter: InputConverter[LongWritable, Text, A])
    extends DataSource[LongWritable, Text, A] {

    private val inputPaths = paths.map(p => new Path(p))

    val inputFormat = classOf[LzoTextInputFormat]
