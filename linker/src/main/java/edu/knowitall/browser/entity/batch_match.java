...
// matching_input_entities.txt - a text file containing the indices being
// considered
// matching_input_context.txt - a text file pointing to the source documents to
// classify into the entities
...
public class batch_match {
  ...
  public batch_match(File basePath) throws CorruptIndexException, IOException {
    ...
    // create a MoreLikeThis using current index
    mlt = new MoreLikeThis(reader);

    this.fbidIndices = new Indices(Constants.derbyDbUrl(Constants.entityLinkingDbPath(basePath)));
  }
  ...
  public void go() throws IOException {
    ...
    for (int a = 0; a < all_arg1.size(); a++) {
      ...
      if ((a > 0) && (a % 250 == 0)) {
        ...
        System.out.println("Finished: " + a + " in " + (millisElapsed / 1000) + " seconds, so "
  ...
  private void processSingleArg(BufferedWriter outputScores, String arg, String sources, String topk)
    ...
    if (likelyDocs.size() == 0) {
      ...
      while (st.hasMoreElements()) {
        ...
        String sourcefile =
            "/scratch/usr/tlin/at_scale/" + char1 + "/" + char2 + "/" + char3 + "/sources/"
                + nextElt + ".data";
      ...
      for (int doc_index = 0; doc_index < inputContext.size(); doc_index++) {
        ...
        List<Pair<String, Double>> results = processSingleArgWithInputStream(arg, likelyDocs, fis);
        String sourceId = sourceFileToId.get(icFilename);
        for (Pair<String, Double> result : results) {
          outputScores.write(arg + "\t" + sourceId + "\t" + result.one + "\t" + result.two + "\n");
        }
  ...
  public List<Pair<String, Double>> processSingleArgWithInputStream(String arg,
      Vector<Integer> likelyDocs, Reader frdr) throws IOException {
  ...
  public Iterable<Pair<String, Double>> processSingleArgWithSources(String arg,
      List<String> likelyFbids, Iterable<String> sources) throws IOException {
    Iterable<Integer> likelyDocs = fbidIndices.getIndices(likelyFbids);
